    ...
    <AnimatePresence>
      {menuType && (
        <motion.div
          ...
          className="absolute inset-0 bg-white shadow-[0_0_40px_rgba(0,0,0,0.05)](0,0,0,0.3)] border-s border-slate-100 pointer-events-auto z-50 overflow-hidden flex flex-col transition-colors duration-300"
        >
           {/* Header */}
              ...
              {menuType === 'history' ? (
                 explorerState.chatHistory?.length > 0 ? (
                     explorerState.chatHistory.map((msg, i) => (
                        <div key={i} className={`p-4 rounded-2xl border ${msg.sender === 'user' ? 'bg-[#f4f7fb] border-[#e2e8f0] ms-8 text-[#333333] shadow-sm' : 'bg-white border-gray-200 text-[#333333] me-8 shadow-sm'}`}>
                           <div className="flex items-center gap-2 mb-2 opacity-60 text-gray-500">
                               <MessageSquare className="w-4 h-4" />
                               <span className="text-[10px] font-bold uppercase tracking-widest">{msg.sender === 'user' ? t('You', 'أنت') : 'GeoVision AI'}</span>
                           </div>
                     ...
                     <div className="flex flex-col items-center justify-center h-full text-slate-400">
                        ...
                        <p className="font-medium tracking-tight">{t('No history yet.', 'لا يوجد سجل حتى الآن.')}</p>
                     </div>
                 )
              ) : (
                 <div className="flex flex-col items-center justify-center h-full text-slate-400">
                    <Bookmark className="w-12 h-12 mb-4 opacity-20" />
                    <p className="font-medium tracking-tight">{t('No saved locations yet.', 'لا توجد مواقع محفوظة حتى الآن.')}</p>
                 </div>
              )}
           </div>
        </motion.div>
